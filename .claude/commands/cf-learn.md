...
- `/project:cf-learn <域>` — 仅扫描指定域相关（如 `scripts`、`cli` 等，以实际存在的域目录为准）
- `/project:cf-learn --map` — 扫描并生成/更新 Retrieval Map（导航地图）
- `/project:cf-learn <域> --map` — 仅生成指定域的导航地图
...
写入目标：
- **全局约束** → 用 Edit 追加到 `CLAUDE.md` 的合适章节
...
与主流程步骤 6 相同：
- 按分类（规则类 / 模式类 / 禁忌类）插入对应 spec 文件的对应章节
- 全局规则写入 `CLAUDE.md` 的合适章节
- 每条写入后输出确认
