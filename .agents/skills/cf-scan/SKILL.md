---
name: cf-scan
description: Audit spec file token distribution, detect redundant or outdated content, and output optimization suggestions. Use when checking spec file health, token budget utilization, or looking for spec files to trim.
---

## 输入

- `cf-scan` — 完整审计
- `cf-scan --json` — 仅输出原始 JSON
- `cf-scan --only-issues` — 仅显示有问题的文件
- `cf-scan --limit=N` — 限制输出行数
